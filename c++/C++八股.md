...
[#define详解](./关键字与限定符/#define.md#define)

`#define`是一个预处理器指令，用于在编译前进行文本替换。它主要用于创建宏定义，可以定义常量、简单的函数或者复杂的表达式。

`#define`为一宏定义语句，本质就是文本替换。

**特点**：

- **文本替换**：`#define`在预处理阶段进行简单的字符串替换，不进行类型检查。
- **宏定义**：它可以定义宏常量，如`#define PI 3.14159`，也可以定义函数样式的宏，如`#define MAX(a, b) ((a) > (b) ? (a) : (b))`。
- **作用域**：宏定义不受作用域限制，除非在某个作用域内重新定义。
- **无类型检查**：在宏替换中不会进行类型检查，可能导致运行时错误。
...
[typedef详解](./关键字与限定符/typedef.md)

`typedef`是`C/C++`语言的关键字，用于创建类型别名，即为已存在的类型起一个新名字。
...
- 类型别名：typedef创建的新类型名称具有完整的类型信息，可以用于类型检查。
- 作用域：typedef定义的类型别名有作用域限制，通常在文件作用域或块作用域中有效。
- 类型安全：typedef定义的别名在编译时会进行类型检查，确保类型使用的正确性。
- 指针操作：typedef在定义指针类型时，可以保持指针的类型完整性，而#define则不能。
...
1. 关键字typedef在编译阶段有效，由于是在编译阶段，因此typedef有类型检查的功能。`#define`则是宏定义，发生在预处理阶段，也就是编译之前，它只进行简单而机械的字符串替换，而不进行任何检查。
